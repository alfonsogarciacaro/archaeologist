"""
Scanner routes - Code analysis and RAG functionality.

This module contains all the scanning and analysis endpoints that were previously
in the separate scanner service, now integrated into the main API service.
"""
...
from ..scanner.rag.models import (
    IngestRequest,
    IngestResponse,
    SearchRequest,
    SearchResponse,
...
async def analyze_dependencies_endpoint(
...
async def impact_analysis_endpoint(
    ...
    scan_result = await scan_code(scan_request, current_user)
...
async def test_scan(current_user: User = Depends(get_current_user)):
    """Test endpoint to verify scanner works with mock data"""
    test_request = ScanRequest(
        query="term_sheet_id",
        paths=["/app/mock_enterprise"]
    )
    return await scan_code(test_request, current_user)

@router.get("/test-impact-analysis")
async def test_impact_analysis(current_user: User = Depends(get_current_user)):
    """Test endpoint for impact analysis"""
    test_request = ImpactAnalysisRequest(
        query="term_sheet_id",
        paths=["/app/mock_enterprise"]
    )
    return await impact_analysis_endpoint(test_request, current_user)

@router.get("/test-ingest")
async def test_ingest(current_user: User = Depends(get_current_user)):
...
''',
    ...
    return await ingest_document(test_request, current_user)

@router.get("/test-search")
async def test_search(current_user: User = Depends(get_current_user)):
    ...
    return await search_documents(test_request, current_user)

@router.get("/test-investigate")
async def test_investigate(current_user: User = Depends(get_current_user)):
    ...
    return await investigate_change(test_request, current_user)